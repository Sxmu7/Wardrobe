  ...
  ready = (async () => {
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS feature_votes (
        id SERIAL PRIMARY KEY,
        feature_id INTEGER NOT NULL REFERENCES feature_ideas(id) ON DELETE CASCADE,
        profile_id INTEGER NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
        created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
        UNIQUE(feature_id, profile_id)
      );
    `;
  })();
  return ready;
...
const DEFAULT_FEATURE_IDEAS = [
  { key: 'kombinieren-v2', title: 'Kombinieren 2.0', description: 'Outfits bauen & durchstoebern kommt ueberarbeitet zurueck.' },
  { key: 'community-v2', title: 'Community 2.0', description: 'Der Freunde-Feed kommt in neuer Form zurueck.' },
  { key: 'kalender', title: 'Kalender & Anlass-Planung', description: 'Outfits im Voraus fuer Termine und Events planen.' },
  { key: 'statistiken', title: 'Statistiken & Insights', description: 'Auswertungen zu deinem Trageverhalten und deinen Lieblingsstuecken.' },
  { key: 'kapsel', title: 'Kapselgarderobe-Planer', description: 'Minimalistische Fits aus wenigen, gut kombinierbaren Teilen.' },
  { key: 'wetter-fit', title: 'Automatischer Wetter-Fit', description: 'Der Tages-Fit passt sich automatisch an Regen und Kaelte an.' },
];

export async function ensureFeatureIdeas() {
  await ensureSchema();
  for (const f of DEFAULT_FEATURE_IDEAS) {
    await sql`
      INSERT INTO feature_ideas (key, title, description) VALUES (${f.key}, ${f.title}, ${f.description})
      ON CONFLICT (key) DO NOTHING;
    `;
  }
}

export async function listFeatureIdeas(viewerProfileId) {
  await ensureFeatureIdeas();
  const { rows } = await sql`
    SELECT
      fi.id, fi.key, fi.title, fi.description, fi.created_at,
      COALESCE(vc.vote_count, 0)::int AS vote_count,
      EXISTS (
        SELECT 1 FROM feature_votes fv WHERE fv.feature_id = fi.id AND fv.profile_id = ${viewerProfileId || 0}
      ) AS voted_by_me
    FROM feature_ideas fi
    LEFT JOIN (
      SELECT feature_id, COUNT(*) AS vote_count FROM feature_votes GROUP BY feature_id
    ) vc ON vc.feature_id = fi.id
    ORDER BY vote_count DESC, fi.created_at ASC;
  `;
  return rows;
}

export async function toggleFeatureVote(featureId, profileId) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT id FROM feature_votes WHERE feature_id = ${featureId} AND profile_id = ${profileId};
  `;
  if (rows.length) {
    await sql`DELETE FROM feature_votes WHERE feature_id = ${featureId} AND profile_id = ${profileId};`;
    return { voted: false };
  }
  await sql`INSERT INTO feature_votes (feature_id, profile_id) VALUES (${featureId}, ${profileId});`;
  return { voted: true };
}
