...
export const db = {
  async addItem(item) {
    const r = await addRecord('items', item);
    import('./sync').then((s) => s.pushItemRemote(item));
    return r;
  },
  getItems: () => getAllRecords('items'),
  async deleteItem(id) {
    const r = await deleteRecord('items', id);
    import('./sync').then((s) => s.deleteItemRemote(id));
    return r;
  },
  async addOutfit(o) {
    const r = await addRecord('outfits', o);
    import('./sync').then((s) => s.pushOutfitRemote(o));
    return r;
  },
  getOutfits: () => getAllRecords('outfits'),
  async deleteOutfit(id) {
    const r = await deleteRecord('outfits', id);
    import('./sync').then((s) => s.deleteOutfitRemote(id));
    return r;
  },
